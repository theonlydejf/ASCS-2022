...
public class ApriltagDetection extends JFrame
{
	...
	private static String[] tagFamilyNames = { "TAG_36h11", "TAG_25h9", "TAG_16h5", "TAG_CIRCLE21h7", "TAG_CIRCLE49h12",
	...
	public ApriltagDetection()
	{
		...
		openButton.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if (returnVal == JFileChooser.APPROVE_OPTION)
				{
					...
					VpImageIo.read(MYIMAGE, file.getAbsolutePath());

					BufferedImage tmp = toBufferedImage(MYIMAGE);
		...
		detectButton.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				if (MYIMAGE.cols() * MYIMAGE.rows() > 0)
				{
					StringBuilder info = new StringBuilder();
					if (detector.detect(MYIMAGE))
					{
						...
						{
							displayLine(canvas, corners.get(0).get_i(), corners.get(0).get_j(), corners.get(1).get_i(),
								corners.get(1).get_j(), Color.RED, 3);
							displayLine(canvas, corners.get(0).get_i(), corners.get(0).get_j(), corners.get(3).get_i(),
								corners.get(3).get_j(), Color.GREEN, 3);
							displayLine(canvas, corners.get(1).get_i(), corners.get(1).get_j(), corners.get(2).get_i(),
								corners.get(2).get_j(), Color.YELLOW, 3);
							displayLine(canvas, corners.get(2).get_i(), corners.get(2).get_j(), corners.get(3).get_i(),
						...
						for (int i = 0; i < tagsId.length; i++)
						{
							...
							displayText(canvas, new String("Id: " + String.valueOf(tagsId[i])), centroid[0] + 10,
		...
		camOk.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				cam = new VpCameraParameters(((Number) pxField.getValue()).doubleValue(),
					((Number) pyField.getValue()).doubleValue(), ((Number) u0Field.getValue()).doubleValue(),
		...
		int thickness)
	{
		...
		displayArrow(I, o.get_y() * cam.get_py() + cam.get_v0(), o.get_x() * cam.get_px() + cam.get_u0(),
			x.get_y() * cam.get_py() + cam.get_v0(), x.get_x() * cam.get_px() + cam.get_u0(), Color.RED, 4 * thickness,
		...
		displayArrow(I, o.get_y() * cam.get_py() + cam.get_v0(), o.get_x() * cam.get_px() + cam.get_u0(),
			z.get_y() * cam.get_py() + cam.get_v0(), z.get_x() * cam.get_px() + cam.get_u0(), Color.BLUE, 4 * thickness,
			2 * thickness, thickness);
	}
